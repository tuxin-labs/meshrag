...
async def test_validate_and_fix_consistency_preserves_chunks_on_reset(tmp_path):
    """FAILED documents are NOT reset to PENDING (terminal state).
    Only PROCESSING documents are reset (crash recovery)."""
    ...
    try:
        ...
        # FAILED documents are NOT reset — they stay as FAILED (terminal state)
        failed_reset = await rag.doc_status.get_by_id(failed_doc_id)
        assert failed_reset is not None
        assert _status_to_text(failed_reset["status"]) == "failed"
        assert failed_reset.get("chunks_list") == ["f-1", "f-2"]
        assert failed_reset.get("chunks_count") == 2

        # PROCESSING documents are reset to PENDING (crash recovery)
        ...
        assert _status_to_text(inferred_count_reset["status"]) == "failed"
        assert inferred_count_reset.get("chunks_list") == ["i-1", "i-2", "i-3"]
        assert inferred_count_reset.get("chunks_count") is None
    finally:
        await rag.finalize_storages()
