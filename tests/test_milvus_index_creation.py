...
class TestMilvusIndexCreation:
    ...
    def test_scalar_index_creation_failure_is_logged_not_raised(self):
        ...
        def create_index_side_effect(*args, **kwargs):
            call_count[0] += 1
            if call_count[0] == 1:
                return None  # 第一次调用（vector index）成功
            raise Exception("Scalar index creation failed")

        with patch.object(storage, "_get_index_params", return_value=mock_index_params):
            mock_client.create_index.side_effect = create_index_side_effect

            # 应完成而不抛出异常（scalar 失败被捕获并记录）
            storage._create_indexes_after_collection()
    ...
    def test_build_index_params_returns_none_when_index_params_is_none(self):
        """Test that build_index_params returns None when index_params is None"""
        config = MilvusIndexConfig(
            index_type="HNSW",
            metric_type="COSINE",
        )

        # 传入 None 时，实际代码直接返回 None，不抛异常
