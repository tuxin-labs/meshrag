...
collect_security_config() {
  ...
  api_key="$(prompt_clearable_secret_with_default "MeshRAG API key: " "${ENV_VALUES[LIGHTRAG_API_KEY]:-}")"
...
finalize_base_setup() {
  ...
  if ((${#DOCKER_SERVICES[@]} > 0)); then
    ...
    svc_names="${svc_names% }"
    echo "MeshRAG requires Docker services: ${svc_names}"
    ...
    if [[ "$effective_default" == "compose" ]]; then
      if ! confirm_default_yes "Run MeshRAG Server via Docker?"; then
      ...
      if confirm_default_no "Run MeshRAG Server via Docker?"; then
        generate_compose="yes"
        runtime_target="compose"
      fi
    fi
