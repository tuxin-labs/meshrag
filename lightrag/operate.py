...
async def _merge_nodes_then_upsert(
    ...
    # Check for cancellation / deletion before LLM summary
    if pipeline_status is not None and pipeline_status_lock is not None:
        async with pipeline_status_lock:
        ...
        async with pipeline_status_lock:
            ...
            if pipeline_status.get("deletion_pending", False):
                ...
                raise PipelineCancelledException(
                    "High priority deletion pending during relation summary"
                )
            if pipeline_status.get("cancellation_requested", False):
                raise PipelineCancelledException(
...
async def merge_nodes_and_edges(
    ...
    # Check for cancellation / deletion at the start of merge
    if pipeline_status is not None and pipeline_status_lock is not None:
        async with pipeline_status_lock:
            # 删除暂停优先，并保留 request_pending 以便删除完成后恢复解析
            if pipeline_status.get("deletion_pending", False):
    ...
    async def _locked_process_entity_name(entity_name, entities):
        async with semaphore:
            # Check for cancellation / deletion before processing entity
            if pipeline_status is not None and pipeline_status_lock is not None:
                async with pipeline_status_lock:
                    # 删除暂停优先，并保留 request_pending 以便删除完成后恢复解析
                    if pipeline_status.get("deletion_pending", False):
                        pipeline_status["request_pending"] = True
                        raise PipelineCancelledException(
                            "High priority deletion pending during entity merge"
                        )
                    if pipeline_status.get("cancellation_requested", False):
                        raise PipelineCancelledException(
    ...
    async def _locked_process_edges(edge_key, edges):
        async with semaphore:
            # Check for cancellation / deletion before processing edges
            if pipeline_status is not None and pipeline_status_lock is not None:
                async with pipeline_status_lock:
                    # 删除暂停优先，并保留 request_pending 以便删除完成后恢复解析
                    if pipeline_status.get("deletion_pending", False):
                        pipeline_status["request_pending"] = True
                        raise PipelineCancelledException(
                            "High priority deletion pending during relation merge"
                        )
                    if pipeline_status.get("cancellation_requested", False):
                        raise PipelineCancelledException(
...
async def extract_entities(
    ...
    # Check for cancellation / deletion at the start of entity extraction
    if pipeline_status is not None and pipeline_status_lock is not None:
        async with pipeline_status_lock:
            # 删除暂停优先，并保留 request_pending 以便删除完成后恢复解析
            if pipeline_status.get("deletion_pending", False):
                pipeline_status["request_pending"] = True
                raise PipelineCancelledException(
                    "High priority deletion pending during entity extraction"
                )
            if pipeline_status.get("cancellation_requested", False):
                raise PipelineCancelledException(
    ...
    async def _process_with_semaphore(chunk):
        async with semaphore:
            # Check for cancellation / deletion before processing chunk
            if pipeline_status is not None and pipeline_status_lock is not None:
                async with pipeline_status_lock:
                    # 删除暂停优先，并保留 request_pending 以便删除完成后恢复解析
                    if pipeline_status.get("deletion_pending", False):
                        pipeline_status["request_pending"] = True
                        raise PipelineCancelledException(
                            "High priority deletion pending during chunk processing"
                        )
                    if pipeline_status.get("cancellation_requested", False):
                        raise PipelineCancelledException(
...
async def kg_query(
    ...
    await _emit_progress(
        ...
        "keyword_extraction",
        {
            "high_level": hl_keywords[:5] if isinstance(hl_keywords, list) else [],
            "low_level": ll_keywords[:5] if isinstance(ll_keywords, list) else [],
        },
    )
...
async def _emit_progress(
    ...
    """安全触发检索进度回调;回调为 None 或回调内异常都不影响检索主流程。"""
    if on_progress is None:
        return
    try:
        await on_progress(stage, detail)
    except Exception as e:  # noqa: BLE001 - 进度回调失败不应中断检索
        logger.debug(f"Progress callback error for stage '{stage}': {e}")
...
async def _build_query_context(
    ...
    await _emit_progress(
        ...
        "vector_search",
        {
            "entities": len(search_result.get("final_entities", [])),
            "relations": len(search_result.get("final_relations", [])),
            "chunks": len(search_result.get("vector_chunks", [])),
        },
    )
    ...
    await _emit_progress(
        ...
        "context_built",
        {
            ...
            "final_chunks": len(raw_data.get("data", {}).get("chunks", [])),
        },
    )
...
async def naive_query(
    ...
    await _emit_progress(
        on_progress,
        "context_built",
        {"final_chunks": len(processed_chunks_with_ref_ids)},
    )
