...
async def apply_rerank_if_enabled(
    ...
    top_n: int = None,
    on_progress: Callable[[str, Optional[dict]], Awaitable[None]] | None = None,
) -> list[dict]:
    """
    ...
    if not enable_rerank or not retrieved_docs:
        if on_progress and not enable_rerank:
            await on_progress("rerank", {"status": "skipped", "reason": "disabled"})
    ...
    if not rerank_func:
        ...
        if on_progress:
            await on_progress(
                "rerank", {"status": "skipped", "reason": "model_not_configured"}
            )
        return retrieved_docs
            ...
            if isinstance(rerank_results[0], dict) and "index" in rerank_results[0]:
                ...
                if on_progress:
                    await on_progress(
                        ...
                        {
                            "status": "success",
                            "kept": len(reranked_docs),
                            "from": len(retrieved_docs),
                        },
                    )
...
def convert_to_user_format(
    ...
    # Filter references to only keep those actually used by entities, relationships, or chunks
    used_reference_ids = set()

    # Collect reference_ids from entities
    for entity in formatted_entities:
        ref_id = entity.get("reference_id")
        if ref_id:
            used_reference_ids.add(ref_id)

    # Collect reference_ids from relationships
    for relation in formatted_relationships:
        ref_id = relation.get("reference_id")
        if ref_id:
            used_reference_ids.add(ref_id)

    # Collect reference_ids from chunks
    for chunk in formatted_chunks:
        ref_id = chunk.get("reference_id")
        if ref_id:
            used_reference_ids.add(ref_id)

    # Filter references and re-number them for continuity
    filtered_references = []
    old_ref_id_to_new: dict[str, str] = {}
    new_ref_id_counter = 1

    for ref in references:
        if ref.get("reference_id") in used_reference_ids:
    ...
    # Update reference_ids in entities, relationships, and chunks
    for entity in formatted_entities:
        old_ref_id = entity.get("reference_id", "")
        if old_ref_id and old_ref_id in old_ref_id_to_new:
            entity["reference_id"] = old_ref_id_to_new[old_ref_id]

    for relation in formatted_relationships:
        old_ref_id = relation.get("reference_id", "")
        if old_ref_id and old_ref_id in old_ref_id_to_new:
            relation["reference_id"] = old_ref_id_to_new[old_ref_id]

    for chunk in formatted_chunks:
        old_ref_id = chunk.get("reference_id", "")
        if old_ref_id and old_ref_id in old_ref_id_to_new:
            chunk["reference_id"] = old_ref_id_to_new[old_ref_id]

    logger.debug(
        f"[convert_to_user_format] Filtered references: {len(filtered_references)}/{len(references)} used, re-numbered for continuity"
    )
