...
# 检索进度回调类型签名:(stage 名称, 可选 detail 字典) -> None
# 用于流式查询时把检索各阶段进度事件推送给前端(模块 A)。
# stage 取值:keyword_extraction / vector_search / merge_chunks /
#             rerank / context_built / retrieval_done
...
class QueryParam:
    ...
    model_func_id: str | None = None
    """用于缓存 key 区分的模型标识符。
    当通过 llm_binding/llm_model 切换模型时，设置为 "{binding}:{model}:{host}" 格式，
    确保不同模型的查询结果不会命中同一缓存。
    """
