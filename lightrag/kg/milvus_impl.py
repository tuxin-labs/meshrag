...
# Milvus connection retry configuration
...
class MilvusIndexConfig:
    ...
    def build_index_params(self, index_params, field_name: str = "vector"):
        ...
        Returns:
            IndexParams object, or None (when index_params is None)
        """
...
class MilvusVectorDBStorage(BaseVectorStorage):
    ...
    def _create_indexes_after_collection(self):
        ...
        if vector_index_params is not None:
            # Custom index configuration provided or AUTOINDEX
            ...
            # This case should now only occur when index_params is None
            logger.warning(
                f"[{self.workspace}] IndexParams not available, skipping vector index creation"
            )
    ...
    def _reconnect(self):
        """Recreate MilvusClient connection"""
        logger.info(f"[{self.workspace}] Recreating MilvusClient connection")
        try:
    ...
    def _create_client(self) -> MilvusClient:
        """Create a new MilvusClient instance with configured timeout."""
        return MilvusClient(
            uri=os.environ.get(
                "MILVUS_URI",
                config.get(
            ...
            timeout=MILVUS_CONNECTION_TIMEOUT,
        )

    async def _create_client_with_retry(self) -> MilvusClient:
        """Create MilvusClient with retry and timeout, without blocking event loop."""
        ...
        wait_strategy = (
            wait_exponential(
                multiplier=MILVUS_CONNECTION_RETRY_BACKOFF,
                min=MILVUS_CONNECTION_RETRY_BACKOFF,
                max=MILVUS_CONNECTION_RETRY_BACKOFF_MAX,
            )
            if MILVUS_CONNECTION_RETRY_BACKOFF > 0
            else wait_fixed(0)
        )
        ...
        async def _before_sleep(retry_state: RetryCallState) -> None:
            ...
            logger.warning(
                "[%s] Milvus connection attempt %s/%s failed: %r",
                self.workspace,
                retry_state.attempt_number,
                MILVUS_CONNECTION_RETRY_ATTEMPTS,
                exc,
            )

        async for attempt in AsyncRetrying(
            stop=stop_after_attempt(MILVUS_CONNECTION_RETRY_ATTEMPTS),
            ...
            with attempt:
                try:
                    client = await asyncio.wait_for(
                        asyncio.to_thread(self._create_client),
                        timeout=MILVUS_CONNECTION_TIMEOUT + 5.0,
                    )
                except asyncio.TimeoutError:
                    raise ConnectionError(
                        f"[{self.workspace}] MilvusClient creation timed out "
                        f"after {MILVUS_CONNECTION_TIMEOUT + 5.0}s"
                    )
                return client
    ...
    async def initialize(self):
        ...
        async with get_data_init_lock():
            ...
            try:
                # Create MilvusClient if not already created (with retry and timeout)
                if self._client is None:
                    logger.info(
                        f"[{self.workspace}] Connecting to Milvus "
                        f"(timeout={MILVUS_CONNECTION_TIMEOUT}s, retries={MILVUS_CONNECTION_RETRY_ATTEMPTS})..."
                    )
    ...
    async def finalize(self):
        """Finalize storage resources and close Milvus connection"""
