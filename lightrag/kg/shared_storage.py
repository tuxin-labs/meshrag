...
async def initialize_pipeline_status(workspace: str | None = None):
    ...
    async with get_internal_lock():
        ...
        # Create a shared list for deletion queue (stores deletion task info)
        deletion_queue = _manager.list() if _is_multiprocess else []
        pipeline_namespace.update(
            {
                ...
                "deletion_queue": deletion_queue,  # 高优先级删除任务队列
                "deletion_pending": False,  # 标记是否有待处理的删除请求
            }
        )
