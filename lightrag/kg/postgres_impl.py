...
from ..exceptions import DataMigrationError, CircuitBreakerOpenError
...
# ============================================================
# PostgreSQL 断路器（Circuit Breaker）
# ============================================================
...
class PostgreSQLCircuitBreaker:
    """
    PostgreSQL 断路器 —— 在连续瞬态故障后快速失败，并定期探测恢复。

    状态转换:
        CLOSED      --[连续失败 ≥ 阈值]--> OPEN
        OPEN        --[恢复超时已过]------> HALF_OPEN
        HALF_OPEN   --[探测成功]----------> CLOSED
        HALF_OPEN   --[探测失败]----------> OPEN

    设计要点:
        - 失败计数在请求级别（非重试级别）：一次 _run_with_retry 调用算 1 次失败
        - 使用独立 asyncio.Lock，不与 _pool_reconnect_lock 共享
        - OPEN 状态下不触及连接池，消除锁竞争
    """
    ...
    async def allow_request(self) -> bool:
        ...
        - CLOSED: 始终允许
        - OPEN: 恢复超时过后允许一个探测请求（转为 HALF_OPEN）
        - HALF_OPEN: 仅允许一个探测请求
        """
        async with self._lock:
            ...
            if self._state == CircuitState.OPEN:
                ...
                if (
                    ...
                    and (now - self._last_failure_time) >= self._recovery_timeout
                ):
                    # 恢复超时已过，转为 HALF_OPEN 并放行一个探测
                    self._state = CircuitState.HALF_OPEN
                    self._half_open_in_progress = True
                    logger.info(
                        "PostgreSQL, Circuit breaker: OPEN -> HALF_OPEN, "
                        "allowing probe request after %.1fs recovery timeout",
            ...
            if self._state == CircuitState.HALF_OPEN:
                if self._half_open_in_progress:
                    return False  # 已有探测请求在飞行中
                self._half_open_in_progress = True
                return True

            return False  # 不可达，防御性编程

    async def record_success(self) -> None:
        """记录成功操作，重置失败计数器。HALF_OPEN 下探测成功则关闭断路器。"""
        async with self._lock:
            if self._state == CircuitState.HALF_OPEN:
                logger.info(
                    "PostgreSQL, Circuit breaker: HALF_OPEN -> CLOSED, "
                    "probe succeeded, PostgreSQL recovered"
                )
            elif self._failure_count > 0:
                logger.debug(
                    "PostgreSQL, Circuit breaker: resetting failure count "
                    "from %d to 0 after successful operation",
                    self._failure_count,
                )
    ...
    async def record_failure(self, exc: Exception) -> None:
        ...
        async with self._lock:
            ...
            if self._state == CircuitState.HALF_OPEN:
                ...
                logger.warning(
                    "PostgreSQL, Circuit breaker: HALF_OPEN -> OPEN, "
                    "probe failed: %r",
                    exc,
                )
                return
            ...
            if self._failure_count >= self._failure_threshold:
                self._state = CircuitState.OPEN
                logger.error(
                    "PostgreSQL, Circuit breaker: CLOSED -> OPEN, "
                    "%d consecutive failures (threshold=%d), "
                    "fast-failing for %.1fs. Last error: %r",
                    self._failure_count,
                ...
                logger.warning(
                    "PostgreSQL, Circuit breaker: failure %d/%d, error: %r",
                    self._failure_count,
                    self._failure_threshold,
                    exc,
                )
    ...
    async def reset(self) -> None:
        """强制重置断路器到 CLOSED 状态（用于 initdb 成功后）。"""
        async with self._lock:
            if self._state != CircuitState.CLOSED:
                logger.info(
                    "PostgreSQL, Circuit breaker: %s -> CLOSED (forced reset)",
                    self._state.value,
                )
            self._state = CircuitState.CLOSED
            self._failure_count = 0
...
class PostgreSQLDB:
    def __init__(self, config: dict[str, Any], **kwargs: Any):
        ...
        self.pool_acquire_timeout = config.get("pool_acquire_timeout", 10.0)
        # 建池阶段的单次 TCP/TLS 连接超时（用于 bootstrap_conn 与 create_pool）
        # 未设置时 asyncpg.connect() 会无限等待，PG 不可达期间探测请求会永久
        # 挂在 _pool_reconnect_lock 里，后续 PG 操作全部超时。故此处强制有限值。
        self.bootstrap_connect_timeout = config.get("bootstrap_connect_timeout", 10.0)
        # TCP keepalive：防止跨网段防火墙/NAT 静默回收空闲连接（0 禁用）
        self.keepalives_idle = config.get("keepalives_idle", 60)
        self.keepalives_interval = config.get("keepalives_interval", 10)
        self.keepalives_count = config.get("keepalives_count", 3)
        # 连接空闲寿命（秒）：asyncpg 主动回收空闲连接的阈值（0 禁用）
        self.max_inactive_connection_lifetime = config.get(
            "max_inactive_connection_lifetime", 120.0
        )
        logger.info(
            "PostgreSQL, Retry config: attempts=%s, backoff=%.1fs, backoff_max=%.1fs, pool_close_timeout=%.1fs, pool_acquire_timeout=%s, bootstrap_connect_timeout=%.1fs",
        ...
        logger.info(
            "PostgreSQL, Connection keepalive: idle=%s, interval=%s, count=%s; max_inactive_connection_lifetime=%ss",
        ...
        # 断路器配置
        self._circuit_breaker = PostgreSQLCircuitBreaker(
            failure_threshold=config["circuit_breaker_failure_threshold"],
            recovery_timeout=config["circuit_breaker_recovery_timeout"],
        )
        logger.info(
            "PostgreSQL, Circuit breaker config: failure_threshold=%s, recovery_timeout=%.1fs",
    ...
    async def initdb(self):
        ...
        # Collect server settings: user-provided values take precedence over
        # the keepalive defaults injected below.
        settings = {}
        ...
        # Inject TCP keepalive defaults (only when not explicitly set by the user).
        # Prevents firewalls/NAT from silently reaping idle connections on cross-subnet links.
        keepalive_defaults = {}
        ...
        async def _create_pool_once() -> None:
            ...
            if self.enable_vector:
                # timeout 防止 PG 不可达时 asyncpg.connect 无限等待，导致
                # _pool_reconnect_lock 被永久持有、后续 PG 操作全部挂起。
            ...
            # 传递单连接建立超时，防止 PG 不可达时建池阶段无限阻塞。
            pool = await asyncpg.create_pool(
                **connection_params,
                init=_init_connection,  # Register pgvector codec on every connection (if enabled)
                timeout=self.bootstrap_connect_timeout,
                # Close idle connections after this many seconds to avoid reusing
                # connections that may have been silently reaped by network middleboxes.
    ...
    async def _run_with_retry(
        ...
        *,
        ...
        集成断路器：在重试循环之前检查断路器状态，OPEN 时快速失败；
        操作成功时记录成功，重试耗尽时记录一次失败。
        ...
        try:
            ...
            # 仅瞬态异常计入断路器失败（一次 _run_with_retry 调用 = 一次断路器失败）
...
def _parse_pool_acquire_timeout() -> float | None:
    """解析 POSTGRES_POOL_ACQUIRE_TIMEOUT 环境变量。

    Returns:
        float | None: 超时秒数，None 表示无限等待。
    """
    raw = os.environ.get("POSTGRES_POOL_ACQUIRE_TIMEOUT", "").strip()
    if not raw:
        return 10.0  # 默认 10 秒
    if raw.lower() == "none":
        return None
    try:
        ...
        logger.warning(
            f"Invalid POSTGRES_POOL_ACQUIRE_TIMEOUT value: '{raw}', using default 10.0"
        )
        return 10.0
...
class ClientManager:
    ...
    def get_config() -> dict[str, Any]:
        ...
        return {
            ...
            # 连接池获取超时：当所有连接都在使用时，新请求等待的最长时间
            # 超时后抛出 asyncio.TimeoutError，触发重试机制
            # 设为 None 表示无限等待（旧行为，不推荐）
            "pool_acquire_timeout": _parse_pool_acquire_timeout(),
            # 建池阶段单次 TCP/TLS 连接超时（bootstrap_conn 与 create_pool）
            # PG 不可达时防止 asyncpg.connect 无限等待，保证 _pool_reconnect_lock
            # 及时释放、断路器能进入下一轮 HALF_OPEN 探测
            "bootstrap_connect_timeout": min(
                60.0,
                max(
                    1.0,
                    float(
                        os.environ.get(
                            "POSTGRES_BOOTSTRAP_CONNECT_TIMEOUT",
                            config.get(
                                "postgres",
            ...
            # TCP keepalive：防止跨网段防火墙/NAT 静默回收空闲连接
            # 设为 0 禁用该项；默认启用（生产友好）
            "keepalives_idle": min(
                86400,
                int(
                    os.environ.get(
                        ...
                        config.get("postgres", "keepalives_idle", fallback=60),
                    )
                ),
            ),
            "keepalives_interval": min(
                3600,
                int(
                    os.environ.get(
                        "POSTGRES_KEEPALIVES_INTERVAL",
                        config.get("postgres", "keepalives_interval", fallback=10),
                    )
                ),
            ),
            "keepalives_count": min(
                100,
                int(
                    os.environ.get(
                        "POSTGRES_KEEPALIVES_COUNT",
                        config.get("postgres", "keepalives_count", fallback=3),
                    )
                ),
            ),
            # 连接空闲寿命：asyncpg 主动回收空闲连接的阈值（秒）
            # 默认 120s（小于 asyncpg 默认 300s），减少命中僵尸连接的概率
            # 设为 0 禁用（回退到 asyncpg 默认行为）
            "max_inactive_connection_lifetime": min(
                86400.0,
                float(
                    os.environ.get(
            ...
            # 断路器配置：连续失败后快速失败，定期探测恢复
            "circuit_breaker_failure_threshold": min(
                50,
                int(
                    os.environ.get(
                        "POSTGRES_CIRCUIT_BREAKER_FAILURE_THRESHOLD",
                        config.get(
                            "postgres",
            ...
            "circuit_breaker_recovery_timeout": min(
                300.0,
                float(
                    os.environ.get(
                        "POSTGRES_CIRCUIT_BREAKER_RECOVERY_TIMEOUT",
                        config.get(
                            "postgres",
...
class PGDocStatusStorage(DocStatusStorage):
    ...
    async def get_doc_id_by_file_path(self, file_path: str) -> Union[str, None]:
        ...
        Returns:
            Union[str, None]: Document ID if found, None otherwise
        """
