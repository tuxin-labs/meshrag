...
def _parse_default_headers_env(env_name: str) -> dict[str, str] | None:
    """Parse a JSON object env var into HTTP header key/value pairs."""
    ...
    try:
        ...
        logger.warning("Invalid JSON in %s: %s", env_name, exc)
        return None

    if not isinstance(parsed, dict):
        logger.warning(
            "%s must be a JSON object, got %s", env_name, type(parsed).__name__
        )
        return None
...
def _merge_openai_default_headers(
    custom_headers: dict[str, str] | None = None,
) -> dict[str, str]:
    """Merge built-in OpenAI client headers with provider-specific headers."""
    headers = {
        "User-Agent": (
            f"Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_8) "
            f"LightRAG/{__api_version__}"
        ),
...
def _apply_openai_client_header_configs(
    client_configs: dict[str, Any] | None,
    *,
    default_headers: dict[str, str] | None = None,
    env_var_name: str | None = None,
) -> dict[str, Any]:
    """Collect custom HTTP headers for AsyncOpenAI client construction."""
    merged_configs = dict(client_configs or {})
    header_updates: dict[str, str] = {}

    if env_var_name:
        env_headers = _parse_default_headers_env(env_var_name)
        if env_headers:
            header_updates.update(env_headers)

    if default_headers:
        header_updates.update(default_headers)

    if header_updates:
        existing_headers = merged_configs.get("default_headers") or {}
        merged_configs["default_headers"] = {**existing_headers, **header_updates}
...
def create_openai_async_client(
    ...
    if use_azure:
        ...
        # 当 LIGHTRAG_SSL_VERIFY=false 时跳过 SSL 证书校验
        # （用于内网自签名证书场景，默认仍校验，生产环境请保持开启）
        if os.getenv("LIGHTRAG_SSL_VERIFY", "true").lower() in ("false", "0", "no"):
            import httpx

            client_configs.setdefault("http_client", httpx.AsyncClient(verify=False))

        custom_headers = client_configs.get("default_headers")
        merged_configs = {
            **client_configs,
            "default_headers": _merge_openai_default_headers(custom_headers),
            "api_key": api_key,
        }
...
async def openai_complete_if_cache(
    ...
    **kwargs: Any,
    ...
    client_configs = _apply_openai_client_header_configs(
        kwargs.pop("openai_client_configs", {}),
        default_headers=kwargs.pop("default_headers", None),
        env_var_name="OPENAI_LLM_DEFAULT_HEADERS",
    )
...
async def openai_embed(
    ...
    client_configs = _apply_openai_client_header_configs(
        client_configs,
        env_var_name="OPENAI_EMBEDDING_DEFAULT_HEADERS",
    )
