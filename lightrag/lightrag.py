...
def _friendly_error_msg_for_exception(e: Exception) -> str:
    """将解析中断/失败异常映射为用户友好的中文提示。

    区分三类：
    1. 被删除任务打断（High priority deletion）：这是正常的调度事件，删除完成后
       会自动恢复解析，不算真失败。
    2. 用户主动取消（User cancelled）：cancel_pipeline 触发，是明确的终态。
    3. 其它异常：保留原始 str(e) 技术细节以便排查。

    Java/前端根据 status=failed + 该 error_msg 展示时，可直接把前两类
    显示为"处理中"或"已中断"等更合理的用户提示，而非"解析失败"。
    """
    from lightrag.exceptions import PipelineCancelledException

    if not isinstance(e, PipelineCancelledException):
        return str(e)
    ...
    return f"文档解析被中断：{msg}"


def _is_deletion_pause_exception(e: Exception) -> bool:
    """判断异常是否由"高优先级删除任务打断"触发（非真失败）。

    被删除打断时文档解析只是暂停，删除完成后会由 apipeline 自动恢复重新处理。
    因此这种情况下**不应标记 FAILED**，应保持 PROCESSING 状态——否则上游
    （Java/前端）轮询到 FAILED 会认为解析失败并停止轮询，而实际解析还在运行。
    """
    from lightrag.exceptions import PipelineCancelledException
    ...
    if not isinstance(e, PipelineCancelledException):
        return False
    msg = str(e)
    return "High priority deletion" in msg or "deletion pending" in msg
...
class LightRAG:
    ...
    async def initialize_storages(self):
        ...
        if self._storages_status == StoragesStatus.CREATED:
            ...
            # 清理孤儿 PROCESSING 文档：服务异常终止后，正在处理的文档状态
            # 停留在 PROCESSING 但实际没有活跃的 pipeline 在运行。
            # 将其重置为 PENDING，并后台触发一次 pipeline 让其继续处理。
            reset_count = await self._reset_orphan_processing_documents()
            if reset_count > 0:
                # 使用 create_task 后台触发，不阻塞其他 KB 的 initialize_storages
                # pipeline 内部会用 busy 标志自锁，重复触发是安全的（幂等）
                asyncio.create_task(self._resume_pipeline_after_orphan_reset())
    ...
    async def drop_storages(self) -> dict[str, list[str]]:
        """Drop all storage data for this LightRAG instance's workspace.

        This method will:
        1. Initialize storages if they are not already initialized
        2. Call drop() on each storage backend
        3. Collect results (success/failure) for each storage
        4. Finalize storages after dropping

        Returns:
            dict[str, list]: Summary of results:
                - 'success': list[str] of successfully dropped storage names
                - 'failed': list[str] of storage names that failed to drop (after retries)
                - 'failed_details': list[dict] with 'storage' and 'error' for each failure
        """
        import shutil
        ...
        # 单个存储 drop 失败时的重试次数。所有后端 drop() 对已删除数据幂等（已验证），
        # 因此失败重试安全；用于覆盖瞬时网络 / 连接故障，尽量把数据清理干净。
        DROP_RETRY_ATTEMPTS = 3

        results = {"success": [], "failed": [], "failed_details": []}

        # All storage instances to drop
        storages = [
            ("llm_response_cache", self.llm_response_cache),
            ("text_chunks", self.text_chunks),
            ("full_docs", self.full_docs),
        ...
        if self._storages_status == StoragesStatus.CREATED:
            logger.debug("Storages not initialized, initializing before dropping")
            await self.initialize_storages()
        elif self._storages_status == StoragesStatus.FINALIZED:
            logger.debug("Storages finalized, re-initializing before dropping")
            await self.initialize_storages()

        def _log_drop_retry(retry_state: RetryCallState) -> None:
            exc = retry_state.outcome.exception() if retry_state.outcome else None
            logger.warning(
                f"[{self.workspace}] Drop storage attempt "
                f"{retry_state.attempt_number}/{DROP_RETRY_ATTEMPTS} failed: {exc!r}"
            )

        # Drop each storage with per-storage retry for reliability
        for storage_name, storage in storages:
            if not storage:
                continue
            try:
                async for attempt in AsyncRetrying(
                    stop=stop_after_attempt(DROP_RETRY_ATTEMPTS),
                    retry=retry_if_exception_type(Exception),
                    wait=wait_exponential(multiplier=1, min=1, max=8),
                    before_sleep=_log_drop_retry,
                    reraise=True,
                ):
                    with attempt:
                        logger.debug(
                        ...
                        if not (
                            isinstance(drop_result, dict)
                            and drop_result.get("status") == "success"
                        ):
                            msg = (
                                drop_result.get("message", "Unknown error")
                                if isinstance(drop_result, dict)
                                else "Unknown error"
                            )
                            raise RuntimeError(f"drop() returned error: {msg}")
                results["success"].append(storage_name)
                logger.debug(
                    f"[{self.workspace}] Successfully dropped storage: {storage_name}"
                )
            except Exception as e:
                results["failed"].append(storage_name)
                results["failed_details"].append(
                    {"storage": storage_name, "error": str(e)}
                )
                logger.error(
                    f"[{self.workspace}] Failed to drop storage {storage_name} "
                    f"after {DROP_RETRY_ATTEMPTS} attempts: {e}"
                )

        # Try to finalize storages (best effort)
        try:
            await self.finalize_storages()
        except Exception as e:
            logger.warning(f"Failed to finalize storages after dropping: {e}")

        # Cleanup file-based storage directory
        if self.workspace:
    ...
    async def apipeline_enqueue_documents(
        ...
        Args:
            ...
            overwrite: 是否覆盖已存在的同内容文档。默认 False：内容重复时创建
                dup-FAILED 记录；设为 True：检测到内容重复时先删除旧文档（含其 chunks/
                实体/关系）再重新入队，用于"文件名不同但内容相同"的覆盖场景。删除未成功
                （如 pipeline 繁忙）时回退到 dup 记录。
        ...
        if ignored_ids:
            ...
            for doc_id in ignored_ids:
                ...
                if overwrite:
                    ...
                    logger.info(
                        f"Overwrite mode: deleting existing document with same content "
                        f"{doc_id} ({file_path})"
                    )
                    try:
                        delete_result = await self.adelete_by_doc_id(doc_id)
                    except Exception as del_err:
                        logger.error(
                            f"Failed to delete existing document {doc_id}: {del_err}"
                        )
                        delete_result = None

                    # success/not_found 表示旧文档已清除（not_found 即文档已被并发删除），
                    # 可安全重新入队，跳过 dup 记录创建
                    if delete_result is not None and delete_result.status in (
                        "success",
                        "not_found",
                    ):
                        unique_new_doc_ids.add(doc_id)
                        logger.info(
                            f"Overwrite mode: document {doc_id} cleared, "
                            f"will re-enqueue for processing"
                        )
                        continue

                    # 删除未成功（如 pipeline 繁忙 not_allowed）→ 回退到创建 dup 记录
                    logger.warning(
                        f"Overwrite delete did not succeed for {doc_id} "
                        f"(status={delete_result.status if delete_result else 'exception'}), "
                        f"falling back to duplicate record"
                    )
    ...
    async def _validate_and_fix_document_consistency(
        ...
        but preserve failed documents.

        Only PROCESSING documents are reset to PENDING (crash recovery).
        FAILED documents are kept as-is — they are terminal and must be
        explicitly reprocessed via /reprocess_failed.
        """
        inconsistent_docs = []
        ...
        # Reset PROCESSING documents that pass consistency checks to PENDING status.
        # FAILED documents are NOT auto-reset — they are terminal and must be
        # explicitly reprocessed via /reprocess_failed (include_failed=True).
        docs_to_reset = {}
        reset_count = 0
        ...
        for doc_id, status_doc in to_process_docs.items():
            ...
            if content_data:  # Document passes consistency check
                # Only reset PROCESSING documents (crash recovery).
                # FAILED documents stay FAILED — they need explicit retry via
                # /reprocess_failed which passes include_failed=True.
                if (
                    hasattr(status_doc, "status")
                    and status_doc.status == DocStatus.PROCESSING
                ):
    ...
    async def _reset_orphan_processing_documents(self) -> int:
        """将孤儿 PROCESSING 文档重置为 PENDING，返回被重置的数量。

        服务异常终止（kill / 崩溃 / 重启）时，正在处理的文档状态停留在
        PROCESSING 但实际没有活跃 pipeline 在运行。下次启动时将这些
        文档重置为 PENDING，让后续解析任务自动恢复处理。
        """
        try:
            processing_docs = await self.doc_status.get_docs_by_status(
            ...
            for doc_id, status_doc in processing_docs.items():
                to_reset[doc_id] = {
                    ...
                    "content_summary": status_doc.content_summary,
                    "content_length": status_doc.content_length,
                    "chunks_count": getattr(status_doc, "chunks_count", -1),
                    "chunks_list": getattr(status_doc, "chunks_list", []),
                    "created_at": status_doc.created_at,
                    "updated_at": datetime.now(timezone.utc).isoformat(),
                    "file_path": getattr(status_doc, "file_path", "unknown_source"),
                    "track_id": getattr(status_doc, "track_id", ""),
                    "error_msg": "Service restarted while processing",
                    "metadata": {},
                }

            await self.doc_status.upsert(to_reset)
            logger.info(
                "Reset %d orphan PROCESSING documents to PENDING (service restart)",
                len(to_reset),
            )
            return len(to_reset)
        except Exception as e:
    ...
    async def _resume_pipeline_after_orphan_reset(self) -> None:
        """服务重启且存在孤儿 PROCESSING 文档时，触发一次 pipeline 让其继续处理。

        在 initialize_storages 收尾用 create_task 后台调用，不阻塞其他 KB 初始化。
        pipeline 内部用 busy 标志自锁，即使被并发触发也是幂等的。
        """
        try:
            logger.info(
                "Auto-triggering pipeline to resume orphan PENDING documents "
    ...
    async def apipeline_process_enqueue_documents(
        ...
        1. Get all pending, and optionally failed/abnormally terminated processing documents.
        ...
        Args:
            assume_busy: 当调用方已持有 pipeline busy（如 background_delete_documents
                完成后无缝切换回解析），跳过初始 busy 检查与状态覆盖，直接进入
                主循环。避免 handoff 期间 busy=False 的空窗，用户看到的
                pipeline_status.busy 始终为 True。
            include_failed: 是否将 FAILED 文档纳入处理队列。默认 False：
                只处理 PROCESSING（崩溃恢复）和 PENDING 文档。设为 True 时
                也会纳入 FAILED 文档，供 /reprocess_failed 等显式重试场景使用。
        """
        ...
        if assume_busy:
            # 调用方已持有 busy（例如 background_delete_documents 完成后接管），
            # 直接读取待处理文档并进入主循环。job_name/busy 已由调用方设置。
            gather_tasks = [
            ...
            to_process_docs: dict[str, DocProcessingStatus] = {}
            to_process_docs.update(results[0])  # PROCESSING
            to_process_docs.update(results[1])  # PENDING
            if include_failed:
                to_process_docs.update(results[2])  # FAILED
            if not to_process_docs:
                logger.info("No documents to process (resumed pipeline)")
                async with pipeline_status_lock:
                    pipeline_status["busy"] = False
                    pipeline_status["latest_message"] = (
                        "No documents to process, pipeline released"
                    )
            ...
            # 检查是否有高优先级删除请求
            async with pipeline_status_lock:
                deletion_pending = pipeline_status.get("deletion_pending", False)
                if deletion_pending:
                    # 设置 request_pending，让删除完成后自动重新触发解析
                    pipeline_status["request_pending"] = True
                    logger.info(
                        "High priority deletion pending, skipping document processing "
                        "(will retry after deletion)"
                    )
                    return
        ...
        try:
            ...
            while True:
                # 检查 pipeline 暂停条件（删除暂停必须优先于用户取消）
                # 说明：deletion_pending 要先于 cancellation_requested 检查。
                # 删除是高优先级操作，暂停时需保留 request_pending=True，以便删除
                # 完成后由 background_delete_documents 重新触发解析；若先检查
                # cancellation_requested 会把 request_pending 清成 False，导致删除
                # 完成后排队中的文档永远不会被处理（任务丢失）。
                async with pipeline_status_lock:
                    # 优先检查高优先级删除请求
                    if pipeline_status.get("deletion_pending", False):
                        log_message = "High priority deletion pending, pausing document processing"
                        logger.info(log_message)
                        pipeline_status["latest_message"] = log_message
                        pipeline_status["history_messages"].append(log_message)
                        # 设置 request_pending 以便删除完成后重新处理
                        pipeline_status["request_pending"] = True
                        return

                    # 再检查用户主动取消（cancel_pipeline）
                ...
                async def process_document(
                    ...
                    async with semaphore:
                        ...
                        try:
                            ...
                            # 删除暂停优先于用户取消，并保留 request_pending 以便删除后恢复解析
                            async with pipeline_status_lock:
                                if pipeline_status.get("deletion_pending", False):
                                    pipeline_status["request_pending"] = True
                            ...
                            # 删除暂停不是失败：保持 PROCESSING，不标 FAILED、不写 error_msg。
                            # 否则上游（Java/前端）轮询到 FAILED 会停止轮询，而实际解析
                            # 还会在删除完成后由 apipeline 自动恢复重新处理。
                            is_deletion_pause = _is_deletion_pause_exception(e)

                            # Check if this is a user cancellation
                            if isinstance(e, PipelineCancelledException):
                                if is_deletion_pause:
                                    # 删除暂停 - 仅记录日志，不写 error_msg
                                    error_msg = f"Paused for deletion {current_file_number}/{total_files}: {file_path}"
                            ...
                            # 删除暂停不标记 FAILED：保持 PROCESSING，让删除完成后
                            # apipeline 恢复时自动重新处理（_validate_and_fix 会把
                            # PROCESSING 重置为 PENDING）。其它异常才标记 FAILED。
                            if not is_deletion_pause:
                        ...
                        if file_extraction_stage_ok:
                            try:
                                ...
                                if isinstance(e, PipelineCancelledException):
                                    if is_deletion_pause:
                                        error_msg = f"Paused for deletion during merge {current_file_number}/{total_files}: {file_path}"
                                        logger.info(error_msg)
                                    else:
                                        # User cancellation - log brief message only, no traceback
                                ...
                                # 删除暂停不标记 FAILED：保持 PROCESSING，让 apipeline
                                # 恢复时自动重新处理。其它异常才标记 FAILED。
                                if not is_deletion_pause:
                                    # Record processing end time for failed case
                                    processing_end_time = int(time.time())
                                    failed_chunks_list, failed_chunks_count = (
            ...
            async with pipeline_status_lock:
                # 若因高优先级删除请求而退出，用更清晰的 job_name 表达当前阶段，
                # 让用户查询 pipeline_status 时看到"正在切换到删除任务"，而非"已停止"。
                # busy 仍设为 False 让 background_delete_documents 阶段1循环立即接管。
                if pipeline_status.get("deletion_pending", False):
                    pipeline_status["busy"] = False
                    pipeline_status["cancellation_requested"] = False
                    handoff_msg = "Pipeline paused, handing off to deletion job"
                    pipeline_status["job_name"] = handoff_msg
                    pipeline_status["latest_message"] = handoff_msg
                    pipeline_status["history_messages"].append(handoff_msg)
                    logger.info(
                        "🔄 Handoff: pipeline paused for high-priority deletion, "
                        "releasing busy for deletion to take over"
                    )
                else:
                    pipeline_status["busy"] = False
    ...
    async def aquery_data(
        ...
        data_param = QueryParam(
            ...
            model_func_id=param.model_func_id,
        ...
        try:
            ...
            if param.mode in ["local", "global", "hybrid", "mix"]:
                ...
                if type(response) is str:
                    if on_progress:
                        await on_progress("retrieval_done", {"status": "bypass"})
                    return {
                        "status": "success",
            ...
            if query_result is None:
                if on_progress:
                    await on_progress(
                        "retrieval_done", {"status": "no_results", "mode": param.mode}
                    )
                return {
                    "status": "failure",
