...
def create_app(args):
    ...
    async def lifespan(app: FastAPI):
        ...
        try:
            # Initialize default KB
            await rag_manager.initialize_default()
    ...
    def discover_knowledge_bases() -> list[str]:
        """Discover KB workspaces based on configured storage type.

        For file-backed storage: scan working directory for subdirectories
        ...
        kb_ids: list[str] = []
        for entry in os.scandir(args.working_dir):
            if entry.is_dir() and not entry.name.startswith("."):
                kb_ids.append(entry.name)
        return kb_ids
    ...
    try:
        ...
        if is_file_based_storage:
            ...
            registry_path = None  # 非文件存储不使用文件 registry
    ...
    async def create_knowledge_base(req: KBRequest):
        await rag_manager.get_rag(req.kb_id)
    ...
    ollama_api = OllamaAPI(rag_manager, top_k=args.top_k, api_key=api_key)
