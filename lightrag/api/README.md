...
git clone https://github.com/tuxin-labs/meshrag.git
...
You can find the docker compose file in the repository root: [docker-compose.yml](../../docker-compose.yml). MeshRAG docker images are published at [ghcr.io/tuxin-labs/meshrag](https://github.com/tuxin-labs/meshrag/pkgs/container/meshrag). For more details about docker deployment, please refer to [DockerDeployment.md](./../../docs/DockerDeployment.md).
...
There are two ways to start multiple LightRAG instances. The first way is to configure a completely independent working environment for each instance. This requires creating a separate working directory for each instance and placing a dedicated `.env` configuration file in that directory. The server listening ports in the configuration files of different instances cannot be the same. Then, you can start the service by running `meshrag-server` in the working directory.
...
```
# Start instance 1
meshrag-server --port 9621 --workspace space1

# Start instance 2
meshrag-server --port 9622 --workspace space2
```
...
# ExecStart=/home/netman/lightrag-xyj/venv/bin/meshrag-server
ExecStart=/home/netman/lightrag-xyj/venv/bin/meshrag-gunicorn
```

> The ExecStart command must be either `meshrag-gunicorn` or `meshrag-server`; no wrapper scripts are allowed. This is because service termination requires the main process to be one of these two executables.
...
After starting the meshrag-server, you can add an Ollama-type connection in the Open WebUI admin panel. And then a model named `lightrag:latest` will appear in Open WebUI's model management interface. Users can then send queries to LightRAG through the chat interface. You should install LightRAG as a service for this use case.
...
```
meshrag-server --llm-binding openai --help
meshrag-server --llm-binding ollama --help
meshrag-server --embedding-binding ollama --help
```
...
## 本仓库改造的接口与配置（Fork API Customizations）

> 本仓库基于上游 LightRAG（基准提交 `0059d2e7`）二次开发，在 API Server 层新增/修改了如下接口与配置项。下列字段均为**可选且向后兼容**：不传时行为与上游单知识库默认一致；上游原有接口与环境变量含义保持不变。改造功能总览见根目录 [README.md](../../README.md) 的「本项目说明」。

### 1. 查询接口增强（`/query`、`/query/stream`、`/query/data`）

三个查询接口共享 `QueryRequest`，新增以下可选字段：

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `kb_ids` | `List[str]` | **多知识库查询**：一次请求同时检索多个本地知识库。流程为「分库并行检索 → 跨库全局去重 → 预算控制 → chunk 重排 → 引用重建 → 生成答案」。不传时使用默认知识库。 |
| `external_kbs` | `List[ExternalKBConfig]` | **外部知识库**列表，可与本地多 KB 混合检索，结构见下表。 |
| `llm_binding` | `str` | **查询级切换 LLM** 提供方：`openai` / `ollama` / `azure_openai` / `gemini` / `aws_bedrock` / `lollms`。提供时 `llm_model` 与 `llm_binding_host` 必填。 |
| `llm_model` | `str` | 临时切换的模型名，如 `gpt-4o`、`qwen-flash`。 |
| `llm_binding_host` | `str` | 临时切换的 LLM API 地址。 |
| `llm_binding_api_key` | `str` | 临时切换的 API Key；不传则复用服务端默认 Key。 |
| `llm_default_headers` | `Dict[str,str]` | 临时自定义 HTTP 头（如华为 ModelArts 的 `X-Apig-AppCode`），与服务端 `OPENAI_LLM_DEFAULT_HEADERS` 合并，同名键以请求值为准。 |

**`ExternalKBConfig` 结构：**

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `type` | `"retrieval"` \| `"rag"` | `retrieval`（检索型）：外部 API 内部做向量检索返回 chunks，由本服务 LLM 生成答案；`rag`（完整 RAG 型）：外部 API 自带 LLM 直接返回最终答案。 |
| `url` | `str` | 外部知识库 API 地址。`retrieval`：接收 `POST {query, top_k}`，返回 `{status, results: [{content, file_path?}]}`；`rag`：接收 `POST {query}`，返回 `{status, answer, references?}`。 |
| `api_key` | `str?` | 外部知识库 API 认证密钥（可选）。 |
| `top_k` | `int?` | 仅 `retrieval` 使用，请求最大结果数，默认 5（范围 1–50）。 |

> 此外提供两个**模拟外部知识库端点**用于联调：转发到 `/query/data`（检索型）与转发到 `/query`（RAG 型）。外部知识库请求失败会透传至响应的 `warnings` 字段，并受超时控制，不会拖垮主流程。
>
> **流式检索进度**：`/query/stream` 在 NDJSON 流中新增 `{"type": "progress", "stage": "...", ...}` 事件，前端可据此判断检索阶段与 rerank 是否生效。

### 2. 文档接口增强（`/documents/*`）

| 能力 | 接口 / 字段 | 说明 |
| --- | --- | --- |
| 上传内容去重 | `/documents/upload`、`/documents/text`、`/documents/texts` | 上传时同步解析内容预检，内容重复直接返回 `duplicated`（响应含 `doc_id`、`original_file_path`）。 |
| 覆盖同名/同内容 | `overwrite: bool`（默认 `false`） | 文档上传/插入支持覆盖已存在的同名与同内容文档。 |
| 文档更新 | `PUT /documents/text/update`、`PUT /documents/texts/update` | 单文档 / 多文档批量更新。 |
| 按文件路径查询 | `GET /documents/by_file_path` | 通过 `kb_id` + `file_path` 快速定位文档与文档 ID，避免文档量大时分页查询低效（JSON / MongoDB / Redis / PostgreSQL 四种后端均实现）。 |
| 本地文件清理 | 内部行为 | 文档解析完成后即删除本地上传文件，节约存储（为后续对接 S3 做准备）。 |

### 3. 改造新增的环境变量

| 变量 | 说明 |
| --- | --- |
| `OPENAI_LLM_DEFAULT_HEADERS` | OpenAI 兼容 LLM 接口的默认自定义 HTTP 头（JSON），如 `{"X-Apig-AppCode": "your-app-code"}`，适配华为 ModelArts 等非 Bearer 认证网关。 |
| `OPENAI_EMBEDDING_DEFAULT_HEADERS` | 同上，作用于 Embedding 接口。 |
| `POSTGRES_CIRCUIT_BREAKER_FAILURE_THRESHOLD` | PostgreSQL 断路器连续失败阈值，默认 5（上限 50）；达到后快速失败，避免数据库崩溃时请求雪崩。 |
| `POSTGRES_CIRCUIT_BREAKER_RECOVERY_TIMEOUT` | 断路器恢复探测间隔（秒），默认 30.0（上限 300）；半开探测成功后自动恢复可用。 |

> 此外，Milvus 在服务启动时若不可用，会进行连接重试并在超时后报错（不再无限阻塞主服务启动）；`pymilvus` 已升级至 `2.6.15` 以修复连接关闭问题。`Dockerfile.backend` 支持可配置多进程部署。
