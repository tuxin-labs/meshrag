...
def create_graph_routes(rag_manager, api_key: Optional[str] = None):
    ...
    async def update_entity(request: EntityUpdateRequest):
        ...
        """
        try:
            rag = await rag_manager.get_rag(request.kb_id)
    ...
    async def update_relation(request: RelationUpdateRequest):
        ...
        try:
            ...
            logger.error(
                f"Validation error updating relation between '{request.source_id}' and '{request.target_id}' in KB {request.kb_id}: {str(ve)}"
            )
            raise HTTPException(status_code=400, detail=str(ve))
        except Exception as e:
            logger.error(
                f"Error updating relation between '{request.source_id}' and '{request.target_id}' in KB {request.kb_id}: {str(e)}"
            )
            logger.error(traceback.format_exc())
            raise HTTPException(
