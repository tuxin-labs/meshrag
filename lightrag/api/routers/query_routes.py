...
class ExternalKBConfig(BaseModel):
    ...
    type: Literal["retrieval", "rag"] = Field(
        default="retrieval",
        description=(
            "外部知识库类型："
            "'retrieval'=检索型，API 内部做向量检索返回相似文本块，由本系统 LLM 生成答案；"
            "'rag'=完整 RAG 服务型，API 内部含 LLM 直接返回最终答案。"
        ),
    )
    url: str = Field(
        description="外部知识库 API 地址。"
        "retrieval 类型：API 接受 POST {query, top_k}，返回 {status, results: [{content, file_path?}]}。"
        "rag 类型：API 接受 POST {query}，返回 {status, answer, references?}。"
    )
    api_key: Optional[str] = Field(
        default=None,
        description="外部知识库 API 的认证密钥（可选）。",
    )
    top_k: Optional[int] = Field(
        default=5,
        ge=1,
        le=50,
        description="仅 retrieval 类型使用。从外部知识库请求的最大结果数量，默认 5。rag 类型忽略此参数。",
    )
...
class QueryRequest(BaseModel):
    ...
    kb_ids: Optional[List[str]] = Field(
        default=None,
        description="List of knowledge base IDs to query. If None, uses the default knowledge base.",
    )

    llm_binding: Optional[str] = Field(
        default=None,
        description="LLM binding type for query override. "
        "Supported: openai, ollama, azure_openai, gemini, aws_bedrock, lollms. "
        "If provided, llm_model must also be provided.",
    )
    llm_model: Optional[str] = Field(
        default=None,
        min_length=1,
        description="LLM model name for query override (e.g., 'gpt-4o', 'qwen-flash'). "
        "If provided, llm_binding must also be provided.",
    )
    llm_binding_host: Optional[str] = Field(
        default=None,
        min_length=1,
        description="LLM API endpoint URL for query override. "
        "Required when providing LLM override parameters.",
    )
    llm_binding_api_key: Optional[str] = Field(
        default=None,
        description="LLM API key for query override. "
        "If not provided, uses the server's default API key.",
    )
    llm_default_headers: Optional[Dict[str, str]] = Field(
        default=None,
        description="Custom HTTP headers for OpenAI-compatible query override "
        "(e.g. Huawei ModelArts AppCode via X-Apig-AppCode). "
        "Merged with OPENAI_LLM_DEFAULT_HEADERS from server env; "
        "query values take precedence for duplicate keys.",
    )

    external_kbs: Optional[List[ExternalKBConfig]] = Field(
        default=None,
        description="外部知识库配置列表。每个外部知识库通过 API 地址检索文档片段，"
        "结果与本地知识库合并后统一 rerank。"
        "支持多个外部知识库并行检索。",
    )
...
class QueryResponse(BaseModel):
    ...
    warnings: Optional[List[str]] = Field(
        default=None,
        description="Warning messages (e.g., external KB timeout/errors)",
    )
...
def _prepare_external_kbs(
    ...
    if not external_kbs:
        return None
    return [ekb.model_dump(exclude_none=True) for ekb in external_kbs]
...
def create_query_routes(
    ...
    def _build_dynamic_model_func(request: QueryRequest):
        ...
        return create_dynamic_llm_func(
            binding=request.llm_binding,
            model=request.llm_model,
            host=request.llm_binding_host,
            api_key=effective_api_key,
            timeout=llm_timeout,
            default_model_kwargs=effective_model_kwargs,
            default_headers=request.llm_default_headers,
        )
    ...
    async def query_text(request: QueryRequest):
        ...
        **Usage Examples:**
        ...
        try:
            ...
            dynamic_func = _build_dynamic_model_func(request)
            if dynamic_func is not None:
                param.model_func = dynamic_func
                param.model_func_id = f"{request.llm_binding}:{request.llm_binding_host}:{request.llm_model}"

            # 当有外部知识库时，允许 kb_ids 为空（仅使用外部 KB）
            ext_kbs = _prepare_external_kbs(request.external_kbs)
            if request.kb_ids:
            ...
            warnings = result.get("metadata", {}).get("external_kb_errors") or None
        ...
        **Error Handling:**
        ...
        try:
            ...
            async def stream_generator():
                # Bridge retrieval progress events into the NDJSON stream
                progress_queue: asyncio.Queue = asyncio.Queue()
                retrieval_task: asyncio.Task | None = None

                async def on_progress(stage: str, detail: dict | None = None):
                    try:
                        detail = detail or {}
                        frame = {
                            "type": "progress",
                        ...
                        # 带 status 的事件(如 rerank 的 success/failed/skipped)把 status
                        # 同时提到顶层,便于前端用 payload.status 直接读取
                        if "status" in detail:
                            frame["status"] = detail["status"]
                        await progress_queue.put(frame)
                    except Exception as e:
                        logger.debug(f"Progress queue put error: {e}")

                try:
                    # Run retrieval concurrently so we can stream progress events
                    retrieval_task = asyncio.create_task(
                        rag_manager.multi_kb_query(
                            request.query,
                            kb_ids,
                            param=param,
                            external_kbs=ext_kbs,
                            on_progress=on_progress,
                        )
                    )

                    # Drain progress events until retrieval_done or task completion
                    while True:
                        try:
                            evt = await asyncio.wait_for(
                                progress_queue.get(), timeout=0.5
                            )
                            yield f"{json.dumps(evt)}\n"
                            if evt.get("stage") == "retrieval_done":
                                break
                        except asyncio.TimeoutError:
                            if retrieval_task and retrieval_task.done():
                                break
                            continue
                ...
                is_bypass = param.mode == "bypass"
                llm_response = result.get("llm_response", {})

                # Bypass mode: no references, only stream LLM response
                if is_bypass:
                ...
                # Non-bypass mode: extract and enrich references
                references = result.get("data", {}).get("references", [])
                ext_kb_warnings = result.get("metadata", {}).get("external_kb_errors")
                ...
                if llm_response.get("is_streaming"):
                    # Streaming mode: send references and warnings first, then stream response chunks
                    if request.include_references or ext_kb_warnings:
                        first_chunk = {}
                        if request.include_references:
                            first_chunk["references"] = references
                        if ext_kb_warnings:
                            first_chunk["warnings"] = ext_kb_warnings
                    ...
                    if ext_kb_warnings:
                        complete_response["warnings"] = ext_kb_warnings
    ...
    # 外部知识库模拟接口的独立 API Key
    EXT_KB_TEST_API_KEY = "ext-kb-test-sk-2026"
    ...
    @router.post(
        "/ext/retrieval",
        tags=["External KB Test"],
        summary="检索型外部知识库模拟接口",
    )
    async def ext_retrieval_endpoint(
        req: ExtRetrievalRequest,
        authorization: str = Header(default=None),
    ):
        """模拟检索型外部知识库 API，转发到内部 /query/data。

        需要通过 Authorization: Bearer {api_key} 认证。
        接受 {"query": "...", "top_k": 5}，
        返回 {"status": "success", "results": [{"content": ..., "file_path": ...}]}。
        默认使用 kb1 知识库。
        """
        # 校验 Bearer Token
        ...
        try:
            rag = await rag_manager.get_rag("kb1")
            ...
            logger.error(f"ext/retrieval 错误: {e}", exc_info=True)
            return {"status": "success", "results": []}

    @router.post(
        "/ext/rag",
        tags=["External KB Test"],
        summary="RAG 型外部知识库模拟接口",
    )
    async def ext_rag_endpoint(
        req: ExtRAGRequest,
        authorization: str = Header(default=None),
    ):
        """模拟 RAG 型外部知识库 API，转发到内部 /query。

        需要通过 Authorization: Bearer {api_key} 认证。
        接受 {"query": "..."}，
        返回 {"status": "success", "answer": "...", "references": [{"file_path": ...}]}。
        默认使用 kb1 知识库。
        """
        # 校验 Bearer Token
        if not authorization or authorization != f"Bearer {EXT_KB_TEST_API_KEY}":
        ...
        try:
            ...
            logger.error(f"ext/rag 错误: {e}", exc_info=True)
            return {"status": "success", "answer": "", "references": []}
