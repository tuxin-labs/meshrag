...
PROMPTS["multi_kb_rag_response"] = """---Role---
...
1. Step-by-Step Instruction:
  ...
  - Scrutinize both `Knowledge Graph Data` and `Document Chunks` in the **Context**. Identify and extract all pieces of information that are directly relevant to answering the user query.
  - Weave the extracted facts into a coherent and logical response. Your own knowledge must ONLY be used to formulate fluent sentences and connect ideas, NOT to introduce any external information.
  - Track the reference_ids of each document chunk which directly support the facts presented in the response. Each chunk contains a list of reference IDs that may originate from different knowledge bases. Correlate all reference IDs with the entries in the `Reference Document List` to generate the appropriate citations.
  - Generate a references section at the end of the response. Each reference document must directly support the facts presented in the response.
  - Do not generate anything after the reference section.
...
PROMPTS["multi_kb_naive_rag_response"] = """---Role---
...
PROMPTS["multi_kb_kg_query_context"] = """
Knowledge Graph Data (Entity):
...
Document Chunks (Each entry contains reference_ids, a list of one or more reference IDs referring to entries in the Reference Document List):
...
PROMPTS["multi_kb_naive_query_context"] = """
Document Chunks (Each entry contains reference_ids, a list of one or more reference IDs referring to entries in the Reference Document List):
...
PROMPTS["rag_answers_merge"] = """---Role---

You are an expert AI assistant tasked with synthesizing answers from multiple knowledge services.

---Goal---

Generate a comprehensive, well-structured answer by combining information from the reference answers below.
Resolve any contradictions by presenting multiple perspectives. Do not fabricate information not present in the source answers.
...
1. Carefully review each reference answer and identify key points.
2. Synthesize the information into a coherent response.
3. If answers contradict each other, present all perspectives with their sources.
4. If all answers agree, provide a unified comprehensive response.
5. The response MUST be in the same language as the user query.
6. The response should be presented in {response_type}.
"""
