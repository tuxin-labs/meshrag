...
class StorageNotInitializedError(RuntimeError):
    ...
    def __init__(self, storage_type: str = "Storage"):
        super().__init__(
            ...
            f"\n"
            f"See: https://github.com/tuxin-labs/meshrag#important-initialization-requirements"
        )
...
class CircuitBreakerOpenError(Exception):
    """PostgreSQL 断路器打开时抛出，表示连接不可用，快速失败不做重试。"""
    ...
    def __init__(
        ...
        super().__init__(
            f"PostgreSQL circuit breaker is {state}. "
            f"Consecutive failures: {failure_count}. "
            f"Requests will be retried after {recovery_timeout:.1f}s recovery timeout."
        )
