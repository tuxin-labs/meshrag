...
const useSettingsStoreBase = create<SettingsState>()(
  persist(
    (set) => ({
      ...
      setSelectedKbId: (selectedKbId: string) => {
        const kb = (selectedKbId || '').trim()
        // Keep strictness: empty kb is invalid, ignore it
        if (!kb) return
        set({ selectedKbId: kb })
      },

      setAvailableKbIds: (availableKbIds: string[]) => {
        const normalized = (availableKbIds || [])
          .map((x) => (x || '').trim())
          .filter((x) => !!x)
        const nextAvailableKbIds = normalized.length ? normalized : ['default']
        const currentSelectedKbId = useSettingsStore.getState().selectedKbId
        set({
          availableKbIds: nextAvailableKbIds,
          selectedKbId: nextAvailableKbIds.includes(currentSelectedKbId)
            ? currentSelectedKbId
    ...
    }),
    {
      ...
      migrate: (state: any, version: number) => {
        ...
        if (version < 20) {
          // Add multi-KB selection defaults
          if (!state.selectedKbId) state.selectedKbId = 'default'
          // availableKbIds is runtime-only, still initialize to keep UI stable
          if (!state.availableKbIds) state.availableKbIds = ['default']
        }
        if (version < 21) {
          if (!Array.isArray(state.availableKbIds) || state.availableKbIds.length === 0) {
            state.availableKbIds = ['default']
          }
