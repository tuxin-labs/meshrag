<!-- Inherited usage documentation, copied from the LightRAG upstream README (CLI names adapted to meshrag-*) (https://github.com/HKUDS/LightRAG), MIT License. -->
<!-- Kept for reference: most content still applies to MeshRAG. See the main README for MeshRAG-specific features. -->
...
## 📝 本项目说明（基于 LightRAG 源码改造）

> 本仓库由上游 [HKUDS/LightRAG](https://github.com/HKUDS/LightRAG) 二次开发而来，**面向后端接口（API Server）场景做了大量定制化改造**，聚焦于多知识库检索、文档管理、查询能力扩展与生产环境稳定性。LightRAG 核心算法（实体/关系抽取、知识图谱构建、local/global/hybrid/mix/naive 多模态检索）保持与上游一致，未做改动。

### 上游源码基准

| 项目 | 内容 |
| --- | --- |
| 上游仓库 | [HKUDS/LightRAG](https://github.com/HKUDS/LightRAG) |
| 基准提交哈希 | `0059d2e7442ecd4d305ae955cc78119800c75291` |
| 基准提交信息 | `Merge branch 'fix-setup-wizard'` |
| 基准提交日期 | 2026-03-12 |
| 基准提交作者 | yangdx |
> 即：本仓库在 `0059d2e7` 之后的所有提交均为本项目的定制化改造，此前内容与上游官方仓库保持一致。

### 改造功能清单

#### 1. 多知识库与外部知识库查询（核心增强）

- **多知识库（multi-KB）查询**：一次请求可同时检索多个本地知识库，由新增的 `RAGManager`（[rag_manager.py](lightrag/api/rag_manager.py)）统一编排；每个 KB 实例独立持有并发参数（如 `MAX_ASYNC`），多 KB / 多 worker 叠加会放大并发量。
- **实现原理（统一后处理）**：参考单知识库内部逻辑，整体流程为「先分库并行检索 → 跨 KB 全局去重 → 预算控制（top_k / token）→ chunk 重排（可选 reranker）→ 引用 / 参考重建 → 生成答案」，从而避免多库结果重复与上下文超预算。
- **外部知识库 API 接入**：新增 [external_kb_client.py](lightrag/api/external_kb_client.py)，支持两类外部知识库：
  - `retrieval`（检索型）：返回文本块 chunks，参与全局重排；
  - `rag`（完整 RAG 型）：直接返回最终答案。
- **内外部知识库混合检索**：本地多 KB 与外部 KB 可在一次查询中组合使用。
- **外部 KB 容错**：请求失败信息透传至查询响应（`warnings` 字段），并加入超时与鉴权控制，避免拖垮主流程。

#### 2. 文档管理增强（[document_routes.py](lightrag/api/routers/document_routes.py)）

- **上传内容去重**：新增 `_check_content_duplicate` + `_extract_text_from_file`，上传时同步解析内容做预检，内容重复直接返回 `duplicated`，`InsertResponse` 扩展 `doc_id` 与 `original_file_path` 字段支撑去重判断。
- **overwrite 覆盖**：文档上传/插入支持覆盖同名与同内容文档。
- **文档更新接口**：新增 `PUT /documents/text/update`（单文档）与 `PUT /documents/texts/update`（多文档批量更新）。
- **按文件路径查询**：新增 `GET /documents/by_file_path`，并在四种存储后端（JSON / MongoDB / Redis / PostgreSQL）实现 `get_doc_id_by_file_path`，解决文档量大时分页查询低效的问题。
- **本地文件清理**：解析完成后即删除本地上传文件，节约本地存储（为后续对接 S3 做准备，原文件统一存 S3）。
- **reference_id 重构**：防止文档 id 重复覆盖；知识库删除接口优化为删除文件、向量并卸载实例；修复文献引用过滤（过滤未引用文献）、文献过滤与排序等问题。
- **文档删除高优先级**：删除请求加入队列机制，pipeline 处理中检测到 `deletion_pending` 时自动暂停文档解析、让删除优先执行；等待超时后强制获取 pipeline 并设置 `cancellation_requested`，确保删除请求不被无限阻塞。
- **文档删除接口增强**：`DELETE /delete_document` 新增预校验（检查 doc_id 是否存在），支持 `partial_not_found` / `not_found` 状态返回，`DeleteDocByIdResponse` 新增 `not_found_ids` 字段。
- **重名上传返回完整信息**：上传重名文件时，`InsertResponse` 正确返回 `doc_id` 与 `original_file_path`，便于前端定位已有文档。

#### 3. 知识库删除可靠性提升

- **drop_storages 单存储重试**：`LightRAG.drop_storages()` 新增 per-storage 级重试（默认 3 次，指数退避），覆盖瞬时网络/连接故障；返回 `failed_details` 列表记录每个失败存储的错误原因，便于排查。
- **KB 删除持久化数据清理**：`RAGManager.delete_knowledge_base()` 当 KB 不在内存时（服务重启 / LRU 淘汰后），自动创建临时 RAG 实例执行 `drop_storages`，确保向量、图谱、文档等持久化数据被可靠清理，不再残留。

#### 4. 查询能力增强（[query_routes.py](lightrag/api/routers/query_routes.py)）

- **单次查询切换 LLM**：Query 接口支持 `llm_binding` / `llm_model` / `llm_binding_host` / `llm_binding_api_key`，请求级临时切换模型与地址。
- **模型感知缓存键**：按模型参数生成缓存键，切换模型时不会意外命中旧缓存。
- **流式检索进度事件**：流式接口新增检索进度事件，前端可直观判断 rerank 是否生效。
- **rerank 链路优化**：端点不可达时缩短重试与超时，避免流式首字长卡顿。

#### 5. 存储层稳定性

- **PostgreSQL 断路器**：新增 `PostgreSQLCircuitBreaker`（[postgres_impl.py](lightrag/kg/postgres_impl.py)），数据库崩溃恢复后服务可自动恢复可用，不再长时间不可用。
- **PostgreSQL 连接池获取超时**：新增 `POSTGRES_POOL_ACQUIRE_TIMEOUT` 环境变量（默认 10 秒），控制连接池满时新请求的等待上限。超时后抛出 `asyncio.TimeoutError` 并触发重试与断路器记录，避免请求无限挂起。
- **Milvus 启动保护**：服务启动时若 Milvus 不可用，新增连接重试超时保护并报错，不再无限阻塞主服务启动。
- **pymilvus 升级**：升级至 `2.6.15`，修复 `Cannot invoke RPC on closed channel` 连接关闭报错与索引创建失败问题。

#### 6. LLM 适配增强（[llm_factory.py](lightrag/api/llm_factory.py)、[openai.py](lightrag/llm/openai.py)）

- **OpenAI 兼容 API 自定义认证头**：新增 `OPENAI_LLM_DEFAULT_HEADERS` / `OPENAI_EMBEDDING_DEFAULT_HEADERS` 环境变量。
- **华为 ModelArts AppCode 等非 Bearer 认证场景**：支持通过 HTTP 头（如 `X-Apig-AppCode`）配置鉴权，适配华为 API 网关等非标准认证体系。
- **query 级覆盖**：Query 接口支持 `llm_default_headers`，临时切换模型时可覆盖地址与认证头。

#### 7. 部署与运维

- **多进程部署**：`Dockerfile.backend` 支持可配置多进程部署；新增 Docker 打包发布流程。
- **国内构建加速**：pip 切换为阿里云源；优化 Dockerfile 的 tiktoken 缓存；忽略 `offline_cache` 分词表离线文件。

#### 8. 测试

- 新增/补全大量单元与集成测试，覆盖 `RAGManager`、query 路由、document 与 overwrite 去重、llm_factory、PostgreSQL 断路器、Milvus 索引、rerank 进度/超时、OpenAI 自定义头、storage 实现等核心改造点。
- **drop_storages 重试测试**（[test_drop_storages.py](tests/test_drop_storages.py)）：覆盖单存储重试、瞬时故障恢复、逻辑失败转异常触发重试等场景。
- **PostgreSQL 连接池获取超时测试**（[test_postgres_pool_acquire_timeout.py](tests/test_postgres_pool_acquire_timeout.py)）：覆盖 timeout 参数传递、超时触发重试、断路器记录、环境变量配置读取等场景。
- **KB 删除临时实例测试**：覆盖 KB 不在内存时创建临时实例清理存储、部分失败返回 `partial_success`、工厂失败仍清理 registry、临时实例不影响 LRU 缓存等场景。

#### 9. WebUI 配套改造（[lightrag_webui/](lightrag_webui/)）

- **知识库切换器**：站点头新增 `KnowledgeBaseSwitcher` 下拉组件，支持列出 / 选择 / 创建知识库，页面上的查询与文档操作自动关联当前选中的 KB。
- **多知识库查询接入**：`QueryRequest` 新增 `kb_ids` 字段，查询自动带上当前选中 KB，请求头携带 `LIGHTRAG-KB` 标识。
- 改动集中在 `SiteHeader.tsx`、`api/lightrag.ts`、`stores/settings.ts`、`QuerySettings.tsx`、`DocumentManager.tsx` 五个文件，WebUI 其余原有功能（知识图谱可视化等）保持不变。

> 说明：以上「改造功能清单」中的每一项均可在本仓库 `0059d2e7` 之后的提交记录与对应源码中找到实现依据；LightRAG 的核心检索算法、上游存储后端等基础能力保持不变。
