...
pip install meshrag[offline]

# Download tiktoken cache
meshrag-download-cache

# Create offline package
pip download meshrag[offline] -d ./offline-packages
...
pip install --no-index --find-links=./offline-packages meshrag[offline]
...
pip install meshrag[api]

# Install API and storage backends
pip install meshrag[api,offline-storage]
...
meshrag-download-cache --cache-dir ./tiktoken_cache

# Download specific models only
meshrag-download-cache --models gpt-4o-mini gpt-4
```
...
meshrag-download-cache --cache-dir ./offline_cache/tiktoken

# 3. Download all Python packages
pip download meshrag[offline] -d ./offline_cache/packages
...
pip install meshrag[offline-storage]

# For LLM providers:
pip install meshrag[offline-llm]
```
...
pip install meshrag[offline]  # Install all offline deps
```
...
1. Check the [GitHub Issues](https://github.com/tuxin-labs/meshrag/issues)
