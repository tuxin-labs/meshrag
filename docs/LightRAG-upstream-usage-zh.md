<!-- 继承文档：复制自 LightRAG 上游 README（CLI 命令名已适配为 meshrag-*）（https://github.com/HKUDS/LightRAG），MIT 许可证。 -->
<!-- 保留供参考：大部分内容仍适用于 MeshRAG。MeshRAG 特有功能见主 README。 -->
...
#### 3. 查询能力增强（[query_routes.py](lightrag/api/routers/query_routes.py)）
...
#### 5. LLM 适配增强（[llm_factory.py](lightrag/api/llm_factory.py)、[openai.py](lightrag/llm/openai.py)）
...
#### 8. WebUI 配套改造（[lightrag_webui/](lightrag_webui/)）
