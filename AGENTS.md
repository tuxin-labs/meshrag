...
MeshRAG is a multi-knowledge-base Graph RAG engine based on LightRAG (HKUDS), enhancing information retrieval and generation through graph-based knowledge representation.
...
- `meshrag-server` or `uvicorn lightrag.api.lightrag_server:app --reload`: start the API locally; ensure `.env` is present.
